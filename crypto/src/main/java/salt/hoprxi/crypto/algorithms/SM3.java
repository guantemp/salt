...
import salt.hoprxi.to.ByteToNumber;
...
public class SM3 {
    ...
    private static byte[] bigEndianIntToByte(int num) {
        return back(ByteToNumber.intToBytes(num));
    }

    private static int bigEndianByteToInt(byte[] bytes) {
        return ByteToNumber.byteToInt(back(bytes));
    }
    ...
    public static byte[] padding(byte[] in, int bLen) {
        ...
        pos += padd.length;
        byte[] tmp = back(ByteToNumber.longToBytes(n));
