 ...
 * @version 0.0.2 2020-04-29
 * @since JDK8.0
 */
public final class PinYin {
    private static final Logger LOGGER = LoggerFactory.getLogger(PinYin.class);
    private static final String CHINESE_CHARACTER_ENCODING = "GBK";
    // 国标一级汉字不同读音的起始区位码对应读音
    private static final char[] FIRST_LETTER = {'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'j', 'k', 'l', 'm', 'n', 'o',
            'p', 'q', 'r', 's', 't', 'w', 'x', 'y', 'z'};
    // 国标一级汉字不同读音的起始区位码
    private static final int[] SEC_POS_VALUE = {1601, 1637, 1833, 2078, 2274, 2302, 2433, 2594, 2787, 3106, 3212, 3472,
            3635, 3722, 3730, 3858, 4027, 4086, 4390, 4558, 4684, 4925, 5249, 5600};
    // 国标二级汉字读音
    private static final String SECOND_SEC_TABLE = "CJWGNSPGCGNE[Y[BTYYZDXYKYGT[JNNJQMBSGZSCYJSYY"
     ...
     * 汉字对应ascii范围
     */
    private static final int[] pinyinValue = new int[]{-20319, -20317, -20304, -20295, -20292, -20283, -20265, -20257, -20242, -20230, -20051, -20036, -20032, -20026, -20002, -19990, -19986, -19982,
    ...
    private static final String[] pinyinStr = new String[]{"a", "ai", "an", "ang", "ao", "ba", "bai", "ban", "bang", "bao", "bei", "ben", "beng", "bi", "bian", "biao", "bie", "bin", "bing", "bo", "bu",
    ...
    public static String toPinYing(String chineseCharacters) {
        ...
        for (int j = 0; j < len; j++) {
            strTemp = chineseCharacters.substring(j, j + 1);
    ...
    private static int getChsAscii(String chs) {
        ...
        try {
            bytes = chs.getBytes(CHINESE_CHARACTER_ENCODING);
        } catch (UnsupportedEncodingException e) {
            if (LOGGER.isDebugEnabled())
                LOGGER.debug("unsupported encoding:" + CHINESE_CHARACTER_ENCODING);
        }
        switch (bytes.length) {
            case 1:
                // 英文字符
     ...
     * @return
     */
    public static String toShortPinYing(String chineseCharacters) {
        if (chineseCharacters == null || chineseCharacters.trim().isEmpty())
            return "";
        StringBuilder buffer = new StringBuilder();
        char ch;
        // 依次处理str中每个字符
        for (int i = 0, j = chineseCharacters.length(); i < j; i++) {
            ch = chineseCharacters.charAt(i);
            byte[] uniCode;
            try {
                uniCode = String.valueOf(ch).getBytes(CHINESE_CHARACTER_ENCODING);
                if (uniCode[0] > 0) { // 非汉字
                    buffer.append(ch);
                } else {
    ...
    /**
     * 获取一个汉字的拼音首字母。 GB码两个字节分别加上96，转换成10进制码组合就可以得到区位码
     */
    private static char convert(byte[] bytes) {
        ...
        if (secPosvalue > 1600 && secPosvalue < 5600) {//一级汉字
            for (int i = 0; i < 23; i++) {
            ...
            int offset = (bytes[0] - 56) * 94 + bytes[1] - 1;
            if (offset >= 0 && offset <= 3007) {//二级汉字3008个
                result = (char) (SECOND_SEC_TABLE.substring(offset, offset + 1).charAt(0) + 32);//+32 转为小写
            }
        }
        return result;
    }
